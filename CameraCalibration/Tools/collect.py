...
parser.add_argument('-path', '--SAVE_PATH', default='../IntrinsicCalibration/data/', type=str, help='Save Video/Image Path')
...
parser.add_argument('-fh','--FRAME_HEIGHT', default=1536, type=int, help='Camera Frame Height')
parser.add_argument('-fps','--VIDEO_FPS', default=30, type=int, help='Camera Video Frame per Second')
...
def main():        
    ...
    cap.set(cv2.CAP_PROP_CONVERT_RGB, 0)                                  # 捕获原始格式
    ...
    while True:                                                                 # 视频输入标定
        ...
        if raw_frame.ndim == 3:
            ...
            if channels == 2:                                                   # YUV格式（如UYVY）
                processed_frame = cv2.cvtColor(raw_frame, cv2.COLOR_YUV2RGB_UYVY)
            else:  # 假设是BGR格式
                processed_frame = raw_frame
        
        if args.DATA_TYPE == 'image':                                           # 【图像采集模式】
            if key == 32:                                                       # 按空格键采集该帧图像     
                img = processed_frame.copy()                                          # 暂时存储该帧图像
                flag = True
            if flag:
